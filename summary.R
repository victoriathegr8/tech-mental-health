...
num_respondees <- tech_data %>%
  nrow()

# Find the average age of the respondees.
...
state_respondees <- tech_data %>%
  group_by(state) %>%
  summarize(num_of_state = length(state)) %>%
  View()

# Find the number of respondees from each company size.
company_size <- tech_data %>%
  group_by(no_employees) %>%
  summarize(num_of_employees = length(no_employees)) %>%
  View()

# How frequently did a respondants mental health interfere with their work?  
work_interfere <- tech_data %>%
  group_by(work_interfere) %>%
  summarize(num_of_responses = length(work_interfere)) %>%
  View()

# How easy did respondants find taking medical leave for mental health to be?
